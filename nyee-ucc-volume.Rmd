...
# MSHS Urgent Care Volume {.tabset .tabset-fade .tabset-pills}
*Report run date: `r report_run_date`*<br/>
*Includes EPIC DEP 310 E 14TH ST URGENT CARE [8164025], 638 COLUMBUS AVE URGENT CARE [8314001], and 310 E 14TH ST URGENT CARE [8087001]*<br/>
...
daily_vol <- access_raw_tbl %>%
  ...
  filter(CONTACT_DATE < TO_DATE(todays_month, "YYYY-MM-DD HH24:MI:SS")) %>%
  ...
  group_by(DEPARTMENT_NAME, DEPARTMENT_ID, appt_year, year_month, CONTACT_DATE) %>%
  ...
  dplyr::select(DEPARTMENT_NAME, DEPARTMENT_ID, appt_year, year_month, CONTACT_DATE, appt_day, total_vol) %>%
  mutate(avg_vol = total_vol) %>%
  filter(!(DEPARTMENT_ID == 8164025 & CONTACT_DATE == "2025-04-08"))
...
reactable(
  ...
  groupBy = c("DEPARTMENT_NAME", "appt_year", "year_month"),
